...
#define fanSpeed 255 //il 7 è 255 resto 200
#define fanTime 350 //il 7 è 2000 resto 500

#define retroTime 30000
...
//PULSANTE
void leggi_pulsante() {
  if (((!digitalRead(buttonPIN)) && primo) || turno) {
    turno = false;
...
//CHECK JEANS
void check_jeans() {
  if ((analogRead(sensorPIN) <= (WET_TRESHOLD - IST)) && (jeansWet == true)) {
    jeansWet = false;
  }
  else if ((analogRead(sensorPIN) > (WET_TRESHOLD + IST)) && (jeansWet == false)) {
    jeansWet = true;
  }
}
